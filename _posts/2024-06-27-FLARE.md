...
s_t = \text{LM}([D\_{q_t}, x, y\_{<t}]) & \text{otherwise} \end{cases}$$
...
$$q\_t = \begin{cases} \emptyset & \text{if all tokens of } \hat{s}\_t \text{ have probs } \geq \theta \\ \text{mask}(\hat{s}\_t) \text{ or } q\_{\text{gen}}(\hat{s}\_t) & \text{otherwise} \end{cases}$$
